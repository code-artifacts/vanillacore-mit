...
- `lockerRegistrySupportsUpdatesFromDistinctAnchors`：不同 anchor 共享的 registry 必须支持并发结构访问。
- `reentrantGrantRemovesWaitRegistration`：重入早返回后不得残留 `txWaitMap`。
...
| --- | ---: | ---: | --- |
| `VC-HEAD-20230430` | 2 | 2 | Yes |
| `VC-REF-95` | 2 | 0 | Yes |

两份 LockTable SHA-256 不同，且 `matrixPassed=true`。这关闭“见证没有实际执行补丁差分”的风险。

## Interpretation

- **Confirmed:** 两个 PR #95 改动都有确定性回归见证，现有默认 suite 未捕获。
- **Confirmed:** 参考补丁同时消除两个见证。
- **Not established:** `ConcurrentHashMap` 是唯一正确修复，或 PR #95 覆盖所有等待/清理路径。
