...
让研究构建不受调用者默认 [`PATH`](../../../scripts/research/common/tooling.py#L229) 影响：所有正式构建固定 Eclipse Temurin JDK 17，并把 JDK 25 仅作为显式兼容性探针。
...
- [`site:bugs.openjdk.org "Invalid CEN header (invalid zip64 extra data field size)"`](#methodology)
- [`site:maven.apache.org enforcer requireJavaVersion`](#methodology)
- [`site:maven.apache.org enforcer requireJavaVendor`](#methodology)
- [`repo.maven.apache.org net.smacke jaydio 0.1`](#methodology)
...
- 没有评估升级 [`jaydio`](#methodology)、排除 JNA 或重打包旧 JAR 的兼容性。

## Implementation

1. [`pom.xml`](../../../pom.xml) 的 [`mit-research`](#implementation) profile 使用 Maven Enforcer 3.6.3，要求 Java `[17,18)` 且 `java.vendor` 精确为 `Eclipse Adoptium`；普通上游构建不被供应商规则污染。
2. [`invoke_maven_jdk17.py`](../../../scripts/research/invoke_maven_jdk17.py) 从显式环境变量、当前 [`java`](https://docs.oracle.com/en/java/javase/17/docs/specs/man/java.html) 与各操作系统标准安装目录解析 JDK 17，验证 runtime 精确为 [`17.0.20+8`](results/step-01-environment.json#L13)，在子进程范围设置 [`JAVA_HOME/PATH`](../../../scripts/research/common/tooling.py#L228) 并强制启用研究 profile。
...
| --- | --- | --- | --- |
| Compiler [`release`](../../../src/test/java/org/vanilladb/core/storage/tx/concurrency/DirectLockTableHarness.java#L80) 约束不是 JDK 选择器 | Confirmed | [Compiler Plugin: set release](https://maven.apache.org/plugins/maven-compiler-plugin/examples/set-compiler-release.html) | Apache Maven 官方文档 |
...
| [`jaydio:0.1`](#evidence) 是 2014 年制品 | Confirmed | [Maven Central directory](https://repo1.maven.org/maven2/net/smacke/jaydio/0.1/) | Maven Central 元数据 |
| 本项目传递解析到 [`jna:4.0.0`](results/step-01-environment.json#L23) | Confirmed | [`mvn dependency:tree`](https://maven.apache.org/plugins/maven-dependency-plugin/tree-mojo.html) 与 [`pom.xml`](../../../pom.xml) | 当前仓库/解析环境 |
...
- JNA 文件 SHA-256 固定为结果 JSON 中的值，JDK 17 [`jar tf`](https://docs.oracle.com/en/java/javase/17/docs/specs/man/jar.html) 成功。
- JDK 25.0.4 对同一文件报告 [`Invalid CEN header (invalid zip64 extra data field size)`](results/step-01-environment.json#L30)；跳过 Enforcer 后 Maven compile 同样失败。
- 因此 JDK 25 限制属于当前依赖制品/ZIP64 校验边界，而不是并发控制实验结果。依赖升级必须成为独立变体。
- [`jdk.util.zip.disableZip64ExtraFieldValidation`](#findings) 只适合作为诊断；[JDK-8328267](https://bugs.openjdk.org/browse/JDK-8328267) 与 [JDK-8363985](https://bugs.openjdk.org/browse/JDK-8363985) 显示 ZipFS/javac 路径仍可能拒绝旧制品。

## Source Inventory

除上表来源外，本轮还审查了：

- [Maven Toolchains guide](https://maven.apache.org/guides/mini/guide-using-toolchains)：说明 toolchain 可让 compiler、Surefire 等插件使用不同于 Maven 进程的 JDK；本步骤不提交机器相关 [`toolchains.xml`](#source-inventory)。
